...
The expected OA HTML source and source-derived inventory were recovered for all
three papers from the registered `v5.2-failed-codex-audit` worktree. Each HTML
file exactly matches the SHA-256 recorded during Day 1. Inventory identity,
source filename, JSON structure, and SHA-256 were checked deterministically.
...
plain-language user tag **Needs human verification**. The 160 exact source
excerpts are retained as unreviewed evidence records, but none is promoted into
an unsupported formulation, arm, or outcome. All papers remain ineligible for
nearest-neighbor and COMET use.

| Paper | Source SHA-256 | Inventory SHA-256 | Preserved evidence records | Bundle status | Formulation/arm/outcome rows |
|---|---|---|---:|---|---:|
...
- Raw provider responses and invocation files were not selected, copied, or
  committed.
- The OA HTML and source inventories were hash-checked in place; their contents
  were not copied into this branch.
- The generated recovery manifest stores logical repository paths and hashes,
  not machine-specific recovery paths.
