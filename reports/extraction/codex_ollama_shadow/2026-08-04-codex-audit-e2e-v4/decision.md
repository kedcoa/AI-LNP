...
The gold-blind run issued all 28 sealed packets with concurrency two. All packets reached terminal accounting: 27 schema-valid responses and one schema failure, with no retries or unattempted packets. The benchmark-time validator accepted 55 of 88 proposals and rejected 33. Required post-run review found that nonnumeric raw values were not required to occur in quoted support. Strict revalidation therefore accepted 46 and rejected 42; nine previously accepted proposals are newly rejected solely as unsupported raw values. That benchmark-time acceptance is a hard safety failure and independently forces `does_not_work`.
...
Safety findings were: gold leakage 0; benchmark-time accepted unsupported/invented facts 9; accepted wrong relationships 0; consecutive systemic failures 0; paid extraction API requests 0; production writes 0; Ollama calls 0. One unused-evidence packet failed local response semantics because it returned proposal disposition without proposals. The validator now rejects nonnumeric raw values that are absent from quoted support, with a regression test; no claim is made that this post-run fix rescues the completed benchmark.
